...
import { User as LucideUser, ArrowBigUp, ArrowBigDown, MessageSquareHeart, UserPlus, Globe, Check, Twitter, Github, MessagesSquare, Linkedin, Instagram, Trash } from 'lucide-react'
...
const User1 = () => {
  ...
  const fetchUser = async () => {
    try {
        const res = await axios.get(`${API_URL}/profile/fetchUser/mongoId/${userId}`)        
  ...
  const fetchPosts = async () => {
    try {
      const res = await axios.get(`${API_URL}/posts/get/${user?.clerkId}`)
  ...
  const requestUpvote = async (postId: string) => {
    try {
      const res = await axios.post(`${API_URL}/posts/upvote/${postId}/${user?.clerkId}`)
  ...
  const requestDownvote = async (postId: string) => {
    try {
      const res = await axios.post(`${API_URL}/posts/downvote/${postId}/${user?.clerkId}`)
  ...
  const fetchTimeline = async () => {
    try {
      const res = await axios.get(`${API_URL}/timeline/get/${user?.clerkId}`)
    ...
    <div className="flex item-center md:flex-row flex-col">
      ...
      <div className="mt-20 p-4 px-2 md:w-1/3 w-full">
        <div className="flex flex-col md:flex-row items-center gap-3">
          ...
          <div className="text-center md:text-left">
            ...
            <div className="flex gap-3 my-1 md:flex-col lg:flex-row">
              <h3 className="font-light">Followers <span className="font-bold">{user?.followerRefs.length}</span></h3>
              <h3 className="font-light">Following <span className="font-bold">{user?.followingRefs.length}</span></h3>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center">About {user?.fullname.split(' ')[0]}<LucideUser size={28} className="mx-2" /></CardTitle>
            </CardHeader>
            <Separator/>
            <div className="p-5">
              {user?.bio}
      ...
      <div className="md:my-20 md:w-2/3 w-full">
        <Tabs defaultValue="timeline" className="px-4">
          <TabsList className="w-full flex justify-evenly">
            <TabsTrigger value="timeline" className="w-full">{user?.fullname.split(' ')[0]}'s Timeline</TabsTrigger>
              ...
              <Card className="w-full my-3" key={post._id}>
                <CardHeader className="flex flex-row gap-4 justify-start items-center">
                  ...
                  <div  className="flex flex-col">
                    ...
                    <Button className="dark:text-gray-300 text-gray-900 justify-start p-0" variant="link" size="sm">{user?.email}</Button>
                  </div>
                </CardHeader>
                <Separator />
                <CardContent>
                ...
                <Separator />
                <div className="flex">
                  <Button onClick={() => requestUpvote(post._id as string)} className={`w-full ${post.upvoteRefs.includes(user?.clerkId as string) ? 'text-red-600' : null}`} variant='ghost' ><ArrowBigUp size={24} className="mx-1" />{post.upvoteRefs.length} Upvotes</Button>
                  <Button onClick={() => requestDownvote(post._id as string)} className={`w-full ${post.downvoteRefs.includes(user?.clerkId as string) ? 'text-red-600' : null}`} variant='ghost'><ArrowBigDown size={24} className="mx-1"/>{post.downvoteRefs.length} Downvotes</Button>
